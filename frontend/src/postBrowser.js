...
function PostBrowser() {
    ...
    function reducer(state, action) {
        if(action.filter === "") {
            ...
            const query = action.filter.toLowerCase();
            let results = [];
            posts.forEach((post) => {
    ...
    const [whitelist, dispatchWhitelist] = useReducer(reducer, []);
                ...
                <div className="browser-topbar">
                    <div className="search-container">
                        <input type="text" className="post-search-bar" ref={searchBar} onKeyUp={(event) =>{
                            if (event.key === "Enter") {
                                dispatchWhitelist({filter: searchBar.current.value});
                            }
